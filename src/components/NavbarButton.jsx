			...
			<AnimatePresence>
				{sectionIndex === index && isIndexNumber && (
					<motion.div
						...
						animate={{ opacity: 1 }}
						exit={{ opacity: 0 }}
						className="absolute bottom-0 w-6 h-1 bg-gray-700 rounded-full sm:-bottom-1"
					></motion.div>
				)}
			</AnimatePresence>
			...
			<AnimatePresence>
				{showSectionName && (
					<motion.div
						...
						exit={{ opacity: 0, y: 10 }}
						className="absolute px-2 py-1 text-sm text-white bg-gray-700 -top-6 sm:-top-8 rounded-xl"
					>
						{sectionName}
					</motion.div>
				)}
